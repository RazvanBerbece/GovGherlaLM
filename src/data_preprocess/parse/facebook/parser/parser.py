...
class ParseRawFacebookHtmlData:
    ...
    def execute(self):
            ...
            for filename in natsort.natsorted(os.scandir(parent), key=lambda x: x.name):
                if filename.is_file():
                    ...
                    message_containers = soup.find_all(
                        "div", class_=message_container_class
                    )
                    ...
                    for container in message_containers:
                        ...
                        content_tags = container.find_all(
                            "div", class_=content_in_container_class
                        )
                        for tag in content_tags:
                            message.content = tag.text

                        # Cleanup content of noise
                        message.content = get_facebook_message_without_reactions(
                            message.content
                        )
                        ...
                        author_tags = container.find_all(
                            "div", class_=author_in_container_class
                        )
        ...
        if self.debugging:
            print(
                f"Extracted {len(messages)} messages and filtered {filtered_messages} in {end - start:0.4f}s"
            )
